...
/**
 * Vista de Generación de Cotizaciones — TechZone (Vendedor / Admin)
 *
 * Permite:
 * - Buscar el cliente por correo o nombre
 * - Agregar productos con cantidad personalizada
 * - Ver el resumen de la cotización con totales
 * - Generar e imprimir el documento de cotización
 *
 * Se embebe como sub-panel en MenuPrincipalView:
 * 
 * <pre>
 * contentPanel.getChildren().add(new CotizacionView(empleado).getRoot());
 * </pre>
 */
public class CotizacionView {
    ...
    public CotizacionView() {
        ...
        this.productosDisponibles = FXCollections.observableArrayList();
    ...
    private void build() {
        ...
        // Columna derecha: tabla cotización + totales
        ...
        colElim.setCellFactory(c -> new TableCell<>() {
            ...
            {
                btn.setStyle(
                        "-fx-background-color: #C83C3C; -fx-text-fill: white; -fx-cursor: hand; -fx-border-width: 0;");
                btn.setOnAction(e -> {
        ...
        gridTotales.add(etiqueta("Desc. ($):"), 0, 1);
        gridTotales.add(txtDescuento, 1, 1);
    ...
    private void cargarProductos() {
        ...
        try {
            var todos = productoServices.obtenerTodosLosProductos();
            for (Producto p : todos) {
                if (Boolean.TRUE.equals(p.getActivo()) && p.getCantidad() > 0) {
                    productosFiltrados.add(p);
                    listProductos.getItems().add(
                            String.format("[%d] %s — $%.2f  (stock: %d)",
                                    p.getIdProducto(), p.getNombre(),
                                    p.getPrecioVenta(), p.getCantidad()));
                }
            }
        } catch (Exception e) {
            listProductos.getItems().add("(Sin conexión a base de datos — datos de ejemplo)");
        }
    }
    ...
    private void buscarCliente() {
        ...
        try {
            Cliente c = personaServices.autenticarCliente(query, "");
            // Si no funciona autenticar por correo, se puede buscar de otra forma
            // Este es un placeholder — implementar personaServices.buscarCliente(query)
            if (c != null) {
                ...
                lblClienteSeleccionado.setText("Cliente no encontrado: " + query);
                lblClienteSeleccionado.setTextFill(Color.web("#C83C3C"));
            }
        } catch (Exception e) {
            lblClienteSeleccionado.setText("Error al buscar cliente.");
            lblClienteSeleccionado.setTextFill(Color.web("#C83C3C"));
        }
    }
    ...
    private void generarCotizacion() {
        if (itemsCotizacion.isEmpty()) {
            info("Agrega al menos un producto.");
            return;
        }

        StringBuilder sb = new StringBuilder();
        sb.append("═══════════════════════════════════════════\n");
        sb.append("           COTIZACIÓN TECHZONE\n");
        sb.append("═══════════════════════════════════════════\n");
        ...
        dlg.setTitle("Cotización generada");
    ...
    private void confirmarVenta() {
        ...
        if (clienteSeleccionado == null) {
            info("Selecciona un cliente antes de confirmar.");
            return;
        }

        Alert conf = new Alert(Alert.AlertType.CONFIRMATION,
                "¿Confirmar la venta por " + lblTotal.getText() + " para " +
        ...
        conf.showAndWait().ifPresent(r -> {
            if (r == ButtonType.YES) {
                try {
                    // TODO: documentoServices.registrarVenta(clienteSeleccionado, itemsCotizacion)
                    info("Venta registrada exitosamente.\n(Conecta documentoServices para persistencia en BD)");
                    limpiarCotizacion();
                } catch (Exception ex) {
